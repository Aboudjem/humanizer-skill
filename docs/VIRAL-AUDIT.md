...
This is a pure-Markdown skill. No executable code, no test runner, nothing to run. The `has_tests` signal (weight 8) can't be satisfied without faking it, which this PR doesn't do.

The right fix: the supernova rubric should detect `repoType === "skill"` and either skip `has_tests` or substitute a skill-specific check — presence of `examples/` with annotated before/after, or a CI step that runs the skill against a sample input via Claude Code. The `examples/blog-post/` directory is the closest functional equivalent: a documented reference case that a CI linter could validate.
...
One candidate false gap: `has_tests` is flagged HIGH for a skill repo, but there's no meaningful test artifact a pure-Markdown skill can produce. The rubric should conditionally downweight or skip this signal when `repoType === "skill"`. This feeds back to the supernova engine iteration log.

No other false gaps. Type detection, packaging recommendation, and signal weights are well-calibrated for this repo.
