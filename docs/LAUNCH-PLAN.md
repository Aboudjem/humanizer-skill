...
### T=0: 13:00 UTC — Primary channel (r/ClaudeAI)

Post to r/ClaudeAI first. This is the highest-fit audience — Claude Code users who write with AI. Title format:
...
### T+1h: 14:00 UTC — Merge awesome-list PRs + publish dev.to post

- Merge any pre-queued awesome-list PRs (awesome-claude-code if approved, others)
...
- Do NOT cold-submit to Hacker News as author — let it happen organically or via a third party

---

## Borrowed-reach strategy

The skill is built for writers and AI-assisted content creators, not just developers. Reach targets:

1. **r/ClaudeAI** — primary; fits perfectly (Claude Code + writing)
2. **Writing/AI Discord servers** — share demo 24h before launch, no hard ask
3. **X writing influencers** — DM 2-3 writers who post about AI writing tools, share the GIF, no ask
4. **dev.to** — underrated origin channel (how superpowers first broke through)
5. **Newsletter** — TLDR AI, Changelog Nightly (submit 48h before launch)

Do NOT cold-Show-HN. If it lands on HN, it'll be because someone else submitted it after seeing the r/ClaudeAI post or the X thread.
...
| Good | 100-200 | X thread gets picked up by a writing influencer |
| Breakout | 500+ | HN third-party submission + multi-channel stacking same day |

The skill is in a sweet spot: specific problem (AI writing patterns), measurable result (0-100 score), free + open source, works in 5 seconds. That's a graspable hook.
...
- Court a writing YouTuber or newsletter (NetworkChuck-equivalent for the writing space)
